...
do 
    bsub -o bert_eval_$frac$size.log -n 8 -W 02:00 -R "rusage[mem=$MEMORY, ngpus_excl_p=1]" -R "select[gpu_model0==$GPU]" python exp/evaluate/selfattn/moleculenet.py --overwrite '{"use_min" : true, "fingerprint" : "cls3"}' --wandb_project moleculenet10-size --model bert$frac-$size-euler 
done
done 
...
for size in 10M
do 
    bsub -o bert_eval_$frac$size.log -n 8 -W 02:00 -R "rusage[mem=$MEMORY, ngpus_excl_p=1]" -R "select[gpu_model0==$GPU]" python exp/evaluate/selfattn/moleculenet.py --overwrite '{"use_min" : true, "fingerprint" : "cls3"}' --wandb_project moleculenet10-size --model bert2$frac-$size-euler 
done
done 
