...
class BarlowLoss(nn.Module):
    def __init__(self, lambd, n_encoding):
        super().__init__()
        self.lambd = lambd
        self.bn = nn.BatchNorm1d(n_encoding, affine=False)
    ...
    def forward(self, z1, z2):
        ...
        c.div_(z1.shape[0])
        #torch.distributed.all_reduce(c) ohne distributed computing brauchen wir das nicht
...
class TrainerBarlow():
    def __init__(self, model, loader, n_encoding, lambd=0.005, validloader=None, metrics={}, metric_pbar_keys=None,
                 input_idxs = [0],
                 output_idxs = [1, 2],
        ...
        self.loss = BarlowLoss(lambd, n_encoding)
        ...
        try:
            ...
            for epoch in range(self.n_epochs):
                ...
                for i, data in enumerate(pbar):
                    ...
                    input1 = to_cuda(data[0])
                    input2 = to_cuda(data[1])
                    z1 = self.model(input1)
                    z2 = self.model(input2)
